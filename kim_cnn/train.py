...
embedding = Embedding(config, TEXT.vocab.vectors)
...
while True:
    ...
    for batch_idx, batch in enumerate(train_iter):
        ...
        model_input = embedding.embed(batch)
        ...
        if iterations % args.dev_every == 1:
            ...
            for dev_batch_idx, dev_batch in enumerate(dev_iter):
                model_input = embedding.embed(dev_batch)
                scores = model(model_input)
            ...
            if dev_acc > best_dev_acc:
                ...
                gpu_snapshot_path = os.path.join(args.save_path, args.dataset, args.mode+'_best_model_gpu.pt')
                cpu_snapshot_path = os.path.join(args.save_path, args.dataset, args.mode+'_best_model_cpu.pt')
                torch.save(model, gpu_snapshot_path)
                model.cpu()
                torch.save(model, cpu_snapshot_path)
